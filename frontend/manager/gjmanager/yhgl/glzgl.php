...
<div class="container">
	...
	<div class="row clearfix">
		...
		<h1>管理者管理</h1>
		<hr/>
		<div style="margin-bottom:5px"><a href="cygl.php" class="btn btn-link">成员管理</a><a href="sjgl2/insert.php" class="btn btn-link">增加新管理者</a><a href="yhgl.php" class="btn btn-link">返回上一级</a><a href="/frontend/manager/gjmanagermain.php" class="btn btn-link">返回后台主界面</a></div>
		<form class="form-inline" method="post" action="">
          <input type="text" name="ss" placeholder="查询管理者" class="form-control"/>
...
//------------------------------------------------------------------------------------------
  ...
  if (isset($ss)||$ss!=null||$ss!="") {
      $sql="select ID,name,sex,place,depart,major,phone,mail from user where name like('%$ss%') or sex like('%$ss%') or place like('%$ss%') or depart like('%$ss%') or major like('%$ss%') or phone like('%$ss%') or mail like('%$ss%') and auth='2' or auth='1'";
  }else{
      $sql="select ID,name,sex,place,depart,major,phone,mail from user where auth='2' or auth='1'";
  }
...
?>
      ...
      <td>
        <form method="post" action="sjgl2/update.php"><input type="hidden" name="id" value="<?php echo $myrow[0]; ?>">
        <button class="btn btn-link" type="submit">修改</button></form>
      </td>
      <td>
        <form method="post" action="/backend/manage/bk_deletemanager.php"><input type="hidden" name="id" value="<?php echo $myrow[0]; ?>">
        <button class="btn btn-link" type="submit" data-toggle="tooltip" data-placement="top" title="快速删除后台管理权限">删除权限</button></form>
      </td>
    </tr>
<?php } ?>
