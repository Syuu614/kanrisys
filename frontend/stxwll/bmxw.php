...
//------------------------------------------------------------------------------------------
  $yhm=$_SESSION['yhm'];
  $sql="select depart from user where yhm='$yhm'";
  $result=mysqli_query($conn,$sql);
  $myrow=mysqli_fetch_array($result);
  $depart=$myrow[0];
  $sql="select ID,ne_title,ne_content from news where ne_belong='$depart'";
  $result=mysqli_query($conn,$sql);
  if (!$result) {
      printf("Error: %s\n", mysqli_error($conn));}
...
?>
    <tr>
      ...
      <form method="post" action="news.php">
        ...
        <span class="shenglue"><?php echo $myrow[2]; ?></span><br/><br/>
      </td>
    </tr>
<?php } ?>
